...
ncomp = 45;      % Numero de componentes MFCC
...
modelorder = 0;  % order of PLP model
...
for i=1:nspk
    ...
    cepstra = melfcc(x, fs, 'wintime',wst,'hoptime',...
        fpt, 'numcep',ncomp, 'nbands', nbands, 'modelorder',modelorder);
    ...
    GMM = fitgmdist(cepstra',ngauss,'RegularizationValue',0.22,...
        'CovarianceType', 'diagonal','Options', statset('MaxIter', 200));
...
for t=1:size(nomlist_test,2)
   ...
   % clean files = 96.8750% accuracy and noisy files = 96.875%
